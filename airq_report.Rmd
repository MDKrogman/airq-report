...
ml_results_glm <- readRDS('models and visuals/results_glm.rds')
```
...
This plot shows the model's consistency in guessing. Note the dashed line, which simply represents the equation $y = x$, or a perfect relationship where the model is 100% accurate (rmse = 0). The consistency at which the model misses the actual hospital demand would indicate bias or improper fit, but I believe this to be the effect of only having one observation per day instead of hourly observations.

# Discussion

Creating models for the prediction of lockdown status and hospital admissions based on this synthetic data provided some insight on the explanatory abilities of certain predictor variables while also helping determine what methods actually have the best predictive ability, robustness, and ability to adapt to unseen data. While the original GLMs were easier to interpret on a variable-by-variable basis, they pale in comparison to the predictive power of the randomforest models using machine learning. That is not to say that those GLMs are a failure–they hone in on the importance of the six common pollutants as predictor variables, and make it clear that they have interactive potential and work together to impact air quality in such a way that prompts reaction in the form of sickness or logistical reactions.

There still exists possibilities for the data to have higher predictive power. In the case of hospital admissions, having hourly data might be more helpful and lead to a lower level of error overall. In its current state, the model can only predict admissions on a daily level. This is helpful without a doubt, and would help hospitals allocate resources, but having hourly predictions could be even more helpful in preparing for waves of demand depending on the time of day–there's not a chance that an equal number of patients are admitted during every hour of the day.  
